...
function drawtopcountriesgraph(topcountries) {
	...
	var x = d3.scale.linear()
		.domain([0, d3.max(topcountries,function(d){return d.chinaexportsovergdp})])
		.range([0, width]);
	...
	var bar = chart.selectAll('g')
		.data(topcountries.sort(function(a,b){return b.chinaexportsovergdp-a.chinaexportsovergdp}))
		.enter().append('g')
		.attr("transform", function (d, i) { return "translate(0," + i * barHeight + ")"; });

	bar.append("rect")
		.attr("width", function(d){
			return x(d.chinaexportsovergdp);
		})
		.attr("height", barHeight - 1);

	bar.append("text")
		.attr("x", function (d) { return x(d.chinaexportsovergdp) - 3; })
		.attr("y", barHeight / 2)
		.attr("dy", ".35em")
